...
let store: AstronomyStore<any>

/**
 * @returns the current or newly created store
 */
const getStore = <TState>(): AstronomyStore<TState> => {
    if(store)
        return store
...
/**
 * Call setStore to provide your own store for redux-astronomy to use. You'll need to use this if you want to use middleware.
 *
 * @param store store to set in the store-registry
 *
 * @returns the current store
 */
const setStore = <TState>(initialStore: AstronomyStore<TState>): AstronomyStore<TState> | null => {
    if(store != null) {
...
const createStore = <TState extends AnyState>(
    ...
    const store: AstronomyStore<TState> = reduxCreateStore(
